...
typedef struct HTreeNode{
	int16_t key;
	uint32_t freq;
	struct HTreeNode* leftNode;
	struct HTreeNode* rightNode;
}HTreeNode;

HTreeNode* buildHuffmanTree(uint32_t *frequence, int16_t startSymbol, uint16_t symbolSize)
{
	HTreeNode * rootNode = NULL;
	if (frequence == NULL || symbolSize == 0)
		return NULL;

	return rootNode;
}

void decodeBitLengthCode(uint8_t * codeSequence, uint8_t codeSequenceLen)
{
#define MAX_CODE_LENGTH_SEQUENCE_LENGTH	19
	uint32_t bl_count[MAX_CODE_LENGTH_SEQUENCE_LENGTH] = { 0 };
	uint32_t bl_freqCount[MAX_CODE_LENGTH_SEQUENCE_LENGTH] = { 0 };
	if (codeSequence == NULL || codeSequenceLen > MAX_CODE_LENGTH_SEQUENCE_LENGTH)
		return;
	for (int i = 0; i < codeSequenceLen; i++)
	{
		bl_count[i] = codeSequence[i];
	}

	for (int i = 0; i < MAX_CODE_LENGTH_SEQUENCE_LENGTH; i++)
	{
		if (codeSequence[i] == 0)
			continue;
		bl_freqCount[codeSequence[i]]++;
	}
	
	printf("Code Freq:");
	for (int i = 0; i < MAX_CODE_LENGTH_SEQUENCE_LENGTH; i++)
	{
		printf("%d ", bl_freqCount[i]);
	}
	printf("\n");

	buildHuffmanTree(bl_freqCount, 0, MAX_CODE_LENGTH_SEQUENCE_LENGTH);
...
int32_t readZipLocalHeader(ZipFile * zipFile, LocalFileHeader * localHeader)
{
	...
	reverseCCL(CCL);
	decodeBitLengthCode(CCL, CCLNum);
